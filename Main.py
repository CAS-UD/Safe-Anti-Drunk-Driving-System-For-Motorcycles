...
req = Send_Requests.SendRequest(url_config, bno055_scl_pin_config, bno055_sda_pin_config, data_number_config, tx_pin_config) 
...
timer.init(period = 10, mode = machine.Timer.PERIODIC, callback = sendData)             
...
async def gpsListening():
    ...
    while True:
        if uart.any():
            req.dat.recordingDataGPS(uart.readline().decode('UTF-8'), time_zone_config)
            while data_number_control != 0:
                pass
